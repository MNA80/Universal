...
-----------------------------------------------
-- Protects the main Teh Universal instance from findfirstchild type attacks
...
do
	...
	EzHub.TextLabel.Text = "Teh Universal"
	...
	EzHub.TextLabel_3.Text = "Universal"
	...
	EzHub.TextLabel_8.Text = "Hello happygeneral2015, Thank you for using Teh Universal"
...
local chosenTheme = _G.TehUniversalTheme or {ThemeIndex = 1};	-- ThemeIndex = 1 signifies that default theme has been chosen
...
loadToStage(0, "Setting up Teh Universal...");
...
local launcherData = game:GetService("HttpService"):JSONDecode(_G["TehUniversalModules"]["launcherdata"]);

-- Usually used as the sponsor box for news display, it is used as a news display for the latest news
...
EzHub.TextLabel_8.Text = "Hello "..game:GetService("Players").LocalPlayer.Name..", Thank you for using Teh Universal";

local ezlib = loadstring(_G["TehUniversalModules"]["ezlib"])();

for i,v in pairs(game:GetService("HttpService"):JSONDecode(_G["TehUniversalModules"]["repostedmodule"])) do
	addScript({
...
for i,v in pairs(game:GetService("HttpService"):JSONDecode(_G["TehUniversalModules"]["exclusivesmodule"])) do
	addScript({
		["scriptName"] = tostring(i),
		["function"] = function()
...
for i,v in pairs(game:GetService("HttpService"):JSONDecode(_G["TehUniversalModules"]["exclusivesv2module"])) do
	addScript({
		["scriptName"] = tostring(i),
		["function"] = function()
			loadstring(game:HttpGet(tostring(v["link"])))();
...
if not _G.TehUniversalExclusives then
	_G.TehUniversalExclusives = {};
end
...
local function closeEzHub()
	...
	if _G.TehUniversalExclusives then
		for i,v in pairs(_G.TehUniversalExclusives) do
	...
	_G.TehUniversalExclusives = nil;

	-- Unload all other variables from global env
	_G.DISABLEEXELOG = nil;
	_G.TehUniversalTheme = nil;
	_G.TehUniversalTerminal = nil;
end

-----------------------------------------------
-- Display how long it took to load Teh Universal
...
-- By using Teh Universal, you automatically agreed to having certain information logged (nothing sensitive)
-- I try to be as open as I can with how I log data - And reasonable.
-- Information of what exactly is logged is stated in the main Teh Universal thread
...
if not _G.DISABLEEXELOG then
	coroutine.wrap(function()
		loadstring(_G["TehUniversalModules"]["logger"])().exeLog();
	end)();
else
	spawn(function() ezlib.newNotif(ezlib.enum.notifType.text, "Teh Universal blocked EXE Logger.").play().delete(); end)
end
]]
...
local localLibPathFile = "TehUniversalLL.txt";
...
loadstring(_G["TehUniversalModules"]["lexermodule"])().highlight_source("Text", EzHub.EditorFrame.Source, EzHub.EditorFrame.Lines);
...
terminalPrint("Loading Teh Universal Terminal...", "y");

local commands = {};	-- stores all commands and their functions
local tips = {
	"Teh Universal Terminal is a built-in command interface for advanced control.",
	"Teh Universal Terminal lets you access many features without using the GUI.",
	"Type cmdlist or one of it's aliases to view all the possible commands.",
	"Teh Universal and the terminal is fully open source! Go to the Github repo to view the source."
};
...
addCommand({"test"}, "Test command to ensure that the terminal is accepting user requests.", function()
	terminalPrint("Teh Universal Terminal is currently functional...", "y");
end);

-------------------------

-- prints the commands to the command list
addCommand({"cmdlist", "list", "cmds", "commands", "cmd"}, "Prints all of the command lists into the Teh Universal terminal.", function()

	terminalPrint("Teh Universal Terminal command list:", "b");
...
addCommand({"quit", "close"}, "Closes Teh Universal and unloads all of it's dependencies.", function()

	terminalPrint("Closing Teh Universal in 3 seconds...", "r");
	spawn(function()
		wait(3);
		closeEzHub();
	end)
...
addCommand({"hide", "hidegui"}, "Hides Teh Universal main panel. Does the same thing as pressing the toggle gui keybind.", function()
	...
	spawn(function()
		...
		terminalPrint("Teh Universal main panel is now hiding.", "b");
	end)
...
addCommand({"guimode"}, "Outputs and accepts input only from the main Teh Universal Terminal GUI.", function()
	if isRConsoleMode then
...
addCommand({"launch", "launchscript"}, "Launches a script from the library of Teh Universal (Exclusives V2).", function(scriptNameArg)
	
	local exclusiveV2s = game:GetService("HttpService"):JSONDecode(_G["TehUniversalModules"]["exclusivesv2module"]);
...
local pluginLinks = game:GetService("HttpService"):JSONDecode(_G["TehUniversalModules"]["pluginLinks"]);
addCommand({"listplugins", "listplugin"}, "Prints a list of verified plugins from the Teh Universal repository.", function()
...
addCommand({"loadplugin"}, "Loads a verified plugin from the Teh Universal repository. To view the list, execute listplugins.", function(pluginNameArg)
...
terminalPrint("Loaded Teh Universal Terminal successfully...", "b");
